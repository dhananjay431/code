    ...
    controller: function($compile) {
      ...
      this.change =function(id){
        ...
        }) == undefined)
        {
          ...
          var index = that.model.findIndex(function(d){ return d == that.op[id]; });
          console.log("else=>",that.model[index]);
          // var td = $(`#${that.model[index].PART_NUMBER} > td`);
          // for(var i=0;i<td.length;i++){
          //   td[i].innerHTML = td[i].innerHTML.replace('<div class="freeze"></div>', "");
          // }
        //  $compile($(`#${that.model[index].PART_NUMBER}`)[0])(that.scope);
        //$(`#${that.model[index].PART_NUMBER}`).show();
        console.log(index);
