angular.module('App.quoteComparisionCtrl')
	.controller('basicCostCtrl', function ($scope, Upload, $window, $log, $state, NgTableParams, cmm) {
        console.log("cmm=>",cmm);
		$scope.data = {};
		$scope.gridOptions = {};
		$scope.cmm = cmm;
		...
		$scope.xl =function(){
            var params ={"skipHeader":false,"columnGroups":true,"skipFooters":false,"skipGroups":false,"skipPinnedTop":false,"skipPinnedBottom":false,"allColumns":true,"onlySelected":false,"suppressQuotes":false,"fileName":"","columnSeparator":""};
			/* if (getBooleanValue(' #appendColumns')) {
				params.customColumns = function(params) {
		...
		$scope.gridOptions2 = {
				...
				},
				{
					...
					cellRenderer: function cellRenderer(params) {
						if (params.data.BASICCOST_BLANKINGDIE == "Excluded") return (params.data.EX_BUDGETED_BASIC_COST);
						else return(params.data.IN_BUDGETED_BASIC_COST);
					}
				},
				{
					...
					valueGetter: 'data.BASICCOST_BLANKINGDIE',
					cellRenderer: function cellRenderer(params) {
						if (params.data.BASICCOST_BLANKINGDIE == "Excluded") return (params.data.EX_BUDGETED_BLANKING_DIE);
						else return (params.data.IN_BUDGETED_BLANKING_DIE);
					}
				},
			],
		...
		/*To add columns dynamically using this colAdd Function */
