...
angular.module('App.quoteComparisionCtrl')
    .controller('dieWeightCtrl', function($scope, Upload, $window, $log, $state, NgTableParams, cmm) {
        console.log("cmm=>",cmm);
        $scope.cmm = cmm;
		...
		$scope.gridOptions2 = {
			...
			columnDefs: [{
				...
				},
				{
					...
					cellRenderer: function cellRenderer(params) {
						if (params.data.DIE_WEIGHT_BLANKINGDIE == "Excluded") return (params.data.EX_DIEWEIGHT_BUDGETED);
						else return (params.data.IN_DIEWEIGHT_BUDGETED);
					}
				}
			],
			components: {
      	...
      	/*This fun is for to change row color acording to condition.*/
		function changeRowColor(params){
			...
			for(var i=0; i<key.length; i++){
				...
				/*if (($scope.CurrentRole == "MSIE")&&(params.STATUS == "QUCO")&& Number(params.data.DIEWEIGHT_BUDGETED) > Number(params.data[key[i]].DIEWEIGHT)){
					return {
                		backgroundColor: '#FFE8E8'
            		}
				}
				else if (($scope.CurrentRole == "MSIE" || $scope.CurrentRole == "Stamping Lead") && (params.data.STATUS == "QUCOSL")) {
					return {
                		backgroundColor: '#FFFFFF'
            		}
				} 
				else if (($scope.CurrentRole == "MSIE" || $scope.CurrentRole == "Stamping Lead" || $scope.CurrentRole == "Asset Manager") && (params.data.STATUS == "QUCOAM")) {
					return {
	...
	/*Function for to change Nodies according to NO_OF_DIES_BLANKINGDIE 
		
		function dieWeightValue(params) {
			var key = _.keys(params.data).filter(function (d) {
				return d.match(/TOOLMAKER[0-9]/i)
			});
			
			if (params.data.DIE_WEIGHT_BLANKINGDIE == 'Included') {
			  return Number(params.data.IN_DIEWEIGHT_BUDGETED);
			} else {
			  return Number(params.data[key[i]].EX_DIEWEIGHT_BUDGETED);
			}
		}
		
		function ChangeKeys(params) {
			var key = _.keys(data[0]).filter(function (d) {
				return d.match(/TOOLMAKER[0-9]/i)
			});
			if (params.data.DIE_WEIGHT_BLANKINGDIE) {
			  return Number(params.data[key[i]].IN_DIEWEIGHT);
			} else {
			  return Number(params.data[key[i]].EX_DIEWEIGHT);
			}
		};*/
		  
	/* WatchCollection is for Save tab*/
...
/**this service for get List of Active users */
        $.cordys.ajax({
...
/**This function is for to receive data of header*/
...
S
	...
	/**Export to excel function start here*/
	$scope.xl =function(){
	...
	/**Export to excel function end here*/
