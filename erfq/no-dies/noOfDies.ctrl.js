angular.module('App.quoteComparisionCtrl')
    .controller('noOfDiesCtrl', function($scope, Upload, $window, $log, $state, NgTableParams, cmm) {
        $scope.data = {};
		...
		columnDefs: [{
			...
			},
			{
				...
				cellRendererParams: function cellRendererParams(params) {
				  var _t = _.map([{key:"Select",value:"Select"},{key:"Included",value:"Included"},{key:"Excluded",value:"Excluded"}], function (d) {
					return {
					  value: d.value,
					  key: d.key
					};
...
document.getElementById("myDropdown").classList.toggle("show");
				...
				if($scope.filterMG==undefined){$scope.filterMG=''}
				if($scope.filterBT==undefined){$scope.filterBT=''}
				if($scope.filterPG==undefined){$scope.filterPG=''}
...
document.getElementById("myDropdown").classList.toggle("show");
      ...
      $scope.downloadFile = function(grid, row, key) {
            ...
            if (row.entity.NO_OF_DIES_BLANKINGDIE == 'Included') {
				//if(row.entity[key]!=undefined && row.entity[key].selected!=undefined){
			...
			for (var i = 0; i < key.length; i++) {
				$scope.gridOptions2.columnDefs.push({
					...
					cellRenderer: function(params) {
						...
						var obj = _.keys(params.data).filter((dd)=>{
