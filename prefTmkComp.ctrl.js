angular.module('App.quoteComparisionCtrl')
    .controller('prefTmkCompCtrl', function($scope, Upload, $window, $log, $state, NgTableParams, cmm, $sce) {
            ...
            var qr = _.flatten(
                $scope.gridOptions.data.map(function(d) {
                    return Object.keys(d).filter(function(d2) {
                        ...
                        if (d.SeqPref != undefined && d.SeqPref[d3.replace("L", "P")]!= undefined && d.SeqPref[d3.replace("L", "P")] != "") {
                            temp.old = {
                                ERFQ_COMPARISON_PREFERENCE: {
        ...
        $scope.tableDataGet = function() {
            ...
            $scope.htp('GetQuoteComparisionbyProject', {
                    ProjectCode: cmm.projectCode,
                    //baselinenum:cmm.baslineNum,
                    baselinenum: cmm.baslineNum,
                    GROUP: ""
        ...
        $scope.GetERFQComparisonData = function() {
            ...
            $scope.htp('GetERFQComparisonData', qr,
                function(data) {
                    ...
                    (
                        $scope.tableArray.map(function(d1) {
        ...
        $scope.UpdateErfqQuoteComparison = function(upDateData) {
            $scope.htp('UpdateErfqQuoteComparison', {
                    "tuple": upDateData
                },
                function(data) {
                ...
                var qr = _.flatten(
                    $scope.gridOptions.data.map(function(d) {
                        return Object.keys(d).filter(function(d2) {
                            ...
                            if (d.ToolmakerPref[d3.replace("L", "P")].PREFERENCESEQ != undefined) {
        ...
        $scope._init = function() {
            ...
            $scope.gridOptions.columnDefs = [{
                ...
                },
                {
                    ...
                    pinnedLeft: true,
                    cellClass: cellClass,
                    cellTemplate: "<div><input type='text' style='width:100%;height:100%;' ng-model='row.entity.JUSTIFICATION'>{{row.entity.JUSTIFICATION}}</div>",
                    width: 150
                }
            ];
        }
        ...
        $scope.add = function(select) {
            var addLength = Object.keys($scope.gridOptions.data[0]).filter(function(d) {
    ...
    .filter('mapGender', function() {
        ...
        return function(input) {
            if (input.TOOLMAKER != undefined)
                return input.TOOLMAKER;
            else
                return input;
        };
    });