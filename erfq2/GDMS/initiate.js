angular .module('app', ['ngMaterial']) .config(function ($mdDateLocaleProvider) {
	...
	.controller("initiatectrl", function ($scope, NgTableParams, Upload, $uibModal, $stateParams, $state) {
		...
		var initNgTable1 = function (data) {
		...
		$scope.changePage1 = function (nextPage1) {
			...
			setTimeout(function () {
						...
						var statusElements = document.querySelectorAll('td[data-title-text="Status"]'); //$$('td[data-title-text="Status"]');

						for (var i = 0; i < statusElements.length; i++) {
							if (statusElements[i].innerHTML == "IN PROGRESS")
								statusElements[i].style.color = "blue";
							else if (statusElements[i].innerHTML == "APPROVED" || statusElements[i].innerHTML == "APPROVED")
		...
		var initNgTable2 = function (data) {
		...
		$scope.initdev = function () {
			...
			//$scope.headerText = ($scope.deviation.GDMS_CODE == "") ? "Create New GDMS" : $scope.deviation.GDMS_CODE;
			$.cordys.ajax({
		...
		$scope.downloadfile = function (fpath, filename) {
			...
			$.cordys.ajax({
				...
				success: function (d) {
					...
					const byteCharacters = atob(d.return);
		...
		$scope.downloadPreview = function () {
			...
			$.cordys.ajax({
				...
				success: function (d) {
					...
					if ($scope.deviation.GDMS_CODE == null || $scope.deviation.GDMS_CODE == undefined)
						filename = $scope.deviation.GDMS_ID + ".pdf";
					const byteCharacters = atob(d.tuple.old.generateGDMSPDF.generateGDMSPDF);
					...
					// var filename = $scope.deviation.GDMS_CODE + ".pdf";
					// var element = document.createElement('a');
					// element.setAttribute('href', 'data:application/octet-stream;charset=utf-16le;base64,' + encodeURIComponent(d.tuple.old.generateGDMSPDF.generateGDMSPDF));
					// element.setAttribute('download', filename);
			...
			if ($scope.FileObject != null) {
				...
				if ("dll" != extension.toLowerCase() && "exe" != extension.toLowerCase()) {
					...
					fileReader.onload = function (fileLoadedEvent) {
						//try {parent.uploadStart()} catch (err) {//}
		...
		$scope.Size = 10240; // allowed file size
		$scope.ViewFileName = function (fileObj, ctr) {
			if (fileObj != null) {
				var fsize = fileObj.size;
				if (ctr == 0)
					document.getElementById("pX").innerHTML = fileObj.name;
				if (ctr == 1)
					document.getElementById("labX").innerHTML = fileObj.name;
		...
		$scope.saveGDMSdetails = function () {
			...
			/*$.cordys.ajax({
				method: "GetGdmsTraGdmsReqObject",
				namespace: "http://schemas.cordys.com/GDMS_WSAppSerPackage",
				dataType: "* json",
				parameters: {
					GDMS_ID: $scope.deviation.GDMS_ID
				...
				success: function (dx) {
					...
					if ($scope.currentStatus.STATUS != "SAVED") {
						alert($scope.currentStatus.GDMS_CODE+ " Already Submitted");
						return;
					}
				...
				$.cordys.ajax({
					...
					success: function (d) {
						...
						if ($scope.deviation.CURRENT_STAGE == "INITIATOR" || !($scope.deviation.CURRENT_STAGE))
							$scope.saveapprovers();
						$scope.savefiledb();
		...
		$scope.saveGDMS = function (status) {
			...
			if (($scope.deviation.CURRENT_STAGE == "INITIATOR" || $scope.deviation.CURRENT_STAGE == undefined) && $scope.deviation.STATUS != "REJECTED") status = "SUBMITTED";
			else status = "REVIEWED";


			// if ($scope.deviation.STATUS == "REJECTED" && status == "SUBMITTED") {                                } else
			...
			if ($scope.deviation.STATUS == "REJECTED") {
				...
				$.cordys.ajax({
					...
					success: function (d) {
						...
						if (status == "SUBMITTED")
							alert($scope.deviation.GDMS_CODE + " Submitted Successfully");
						else
							toastr.success("Data Saved Successfully");
					},
					fail: function (jqXHR, textStatus, errorThrown) {
		...
		$scope.generatedevtuple = function () {
			...
			for (var i = 0; i < $scope.deliverables.length; i++) {
				...
				if ($scope.deliverables[i].START_DATE != null)
					if (typeof ($scope.deliverables[i].START_DATE) == "string")
						st = $scope.deliverables[i].START_DATE;
					else
						st = $scope.deliverables[i].START_DATE.getFullYear() + "-" + ($scope.deliverables[i].START_DATE.getMonth() + 1) + "-" + $scope.deliverables[i].START_DATE.getDate();

				if ($scope.deliverables[i].TARGET_IMPLE_DATE != null)
					if (typeof ($scope.deliverables[i].TARGET_IMPLE_DATE) == "string")
						tid = $scope.deliverables[i].TARGET_IMPLE_DATE;
					else
						tid = $scope.deliverables[i].TARGET_IMPLE_DATE.getFullYear() + "-" + ($scope.deliverables[i].TARGET_IMPLE_DATE.getMonth() + 1) + "-" + $scope.deliverables[i].TARGET_IMPLE_DATE.getDate();
		...
		$scope.deleteRow = function () {
			...
			for (i = len - 1; i >= 0; i--) {
				...
				if ($scope.deliverables[i].selected == true) {
					if ($scope.deliverables[i].DEVIATION_ID != undefined)
						$scope.deleteddeviations.push($scope.deliverables[i].DEVIATION_ID);
		...
		$scope.deletefileRow = function () {
			...
			for (i = len - 1; i >= 0; i--) {
				...
				if ($scope.download[i].selected == true) {
					if ($scope.download[i].DOCUMENT_ID != undefined)
						$scope.deletedfile.push($scope.download[i].DOCUMENT_ID);
		...
		$scope.deleteClrFileRow = function () {
			...
			for (i = len - 1; i >= 0; i--) {
				...
				if ($scope.clrdownload[i].selected == true) {
					if ($scope.clrdownload[i].DOCUMENT_ID != undefined)
						$scope.deletedfile.push($scope.clrdownload[i].DOCUMENT_ID);
		...
		$scope.pqhchange = function () {
			if ($scope.pqh == false) $scope.pqh = true;
			else $scope.pqh = false;
			$scope.appr.pqhead = null;
		}
		$scope.adpdqhchange = function () {
			if ($scope.adpdqhc == false) $scope.adpdqhc = true;
			else $scope.adpdqhc = false;
			$scope.appr.adpdqh = null;
		}
		$scope.adpdchange = function () {
			if ($scope.adpd == false) $scope.adpd = true;
			else $scope.adpd = false;
			$scope.appr.adpdh = null;
		}
		$scope.cdmmhchange = function () {
			if ($scope.cdmmh == false) $scope.cdmmh = true;
			else $scope.cdmmh = false;
			$scope.appr.cdmmhead = null;
		}
		$scope.ccheadchange = function () {
			if ($scope.cch == false) $scope.cch = true;
			else $scope.cch = false;
			$scope.appr.cchead = null;
		}
		$scope.ahphchange = function () {
			if ($scope.ahphc == false) $scope.ahphc = true;
			else $scope.ahphc = false;
			$scope.appr.ahph = null;
		}
		$scope.ohchange = function () {
			if ($scope.oh == false) $scope.oh = true;
			else $scope.oh = false;
			$scope.appr.operationhead = null;
		}
		$scope.pphchange = function () {
			if ($scope.pph == false) $scope.pph = true;
			else $scope.pph = false;
		...
		$scope.workflowHis = function () {
			...
			$.cordys.ajax({
				...
				success: function (e) {
					...
					/*for (var i = 0; i < $scope.res2.length; i++) {
                        if ($scope.res2[i].DECISION == "IN PROGRESS")
                            $scope.res2[i].colortxt = "blue";
                        else if ($scope.res2[i].DECISION == "APPROVED" || $scope.res2[i].DECISION == "SUBMITTED")
                            $scope.res2[i].colortxt = "green";
                        else
                            $scope.res2[i].colortxt = "red";
                    }
					*/
		...
		$scope.Getgdmsreqbyno = function () {
			$.cordys.ajax({
				...
				success: function (d) {
					...
					if ($scope.deviation.CURRENT_STAGE != 'INITIATOR' || $scope.deviation.STATUS == 'REJECTED')
						$scope.gettaskid();
					...
					if ($scope.deviation.CURRENT_STAGE == "PRESIDENT" || !($scope.deviation.CURRENT_STAGE))
						$scope.buttonname = "Approve";
					else if ($scope.deviation.CURRENT_STAGE == "QUALITY HEAD" || !($scope.deviation.CURRENT_STAGE))
						$scope.buttonname = "Review";
					else
						$scope.buttonname = "Submit Request";
